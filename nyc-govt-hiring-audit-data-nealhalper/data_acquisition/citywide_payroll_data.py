...
class NYCPayrollDataFetcher(BaseDataPipeline):
    def __init__(self):
        super().__init__("nyc_payroll")
        self.base_url = os.getenv('NYC_PAYROLL_API_URL')
        if not self.base_url:
            raise ValueError("NYC_PAYROLL_API_URL not found in environment variables")
    ...
    def fetch_data(self, max_rows: Optional[int] = None, delay: float = None, use_cache: bool = True) -> pl.DataFrame:
        """
        Fetch payroll data with caching and raw data storage
        ...
        print(f"Starting NYC Payroll data fetch...")
        if max_rows:
        ...
        if all_data:
            ...
            self.save_metadata(metadata, f"payroll_fetch_{Config.get_timestamp()}")
            
            df = pl.DataFrame(all_data)
            print(f"Raw DataFrame shape: {df.shape}")
            print("Note: Data is unprocessed. Use process_payroll_data.py to clean and process.")
            return df
        else:
            return pl.DataFrame()

def main():
    """Main function to fetch NYC payroll data (no processing)"""
    fetcher = NYCPayrollDataFetcher()
    
    # Development mode with caching
    print("=== FETCHING ONLY: Getting first 10,000 rows ===")
    ...
    if not df_raw.is_empty():
        ...
        print("\nTo process this data, run: python data_processing/process_payroll_data.py")
